...
- Shared text transport: `call_gemini()` — `api/index.py:795-843`

### 1.1 Shared transport — `call_gemini()`

**File/line:** `api/index.py:795`

**Request built** (`api/index.py:806-819`):
...
Response parsed at `api/index.py:834-843`. Raises `HTTPException(502)` on non-200,
unexpected shape, or empty completion.

### 1.2 Shared style/length guidance

`PODCAST_STYLE_GUIDANCE` (`api/index.py:866`) and `SUMMARY_LENGTH_GUIDANCE`
(`api/index.py:884`) back both the combined analysis prompt and the standalone regenerate
...
**Handler:** `api/index.py:1230` · **Frontend:** `src/App.jsx:329`

Multipart form: `file: UploadFile` (PDF ≤4 MB, `read_pdf_upload()` at `api/index.py:1069`)
plus optional `podcast_style` / `summary_length` / `summary_focus` Form fields and the
`Authorization` header.

**Prompt** built by `build_study_system_instruction()` (`api/index.py:904-934`) — a single
...
Called at `api/index.py:1265`. Normalised by `normalise_study_content()`
(`api/index.py:1051`) via `parse_summary_points()` (`api/index.py:1005`),
`parse_quiz_questions()` (`api/index.py:979`), `parse_podcast_script()` (`api/index.py:1011`,
...
**Handler:** `api/index.py:1308` · **Frontend:** `src/App.jsx:1318` (`TutorPanel.send()`)
...
**Handler:** `api/index.py:1389` · **Frontend:** `src/App.jsx:801`

Prompt `QUIZ_SYSTEM_INSTRUCTION` (`api/index.py:962`). Avoid-list = current quiz + last 5
...
**Handler:** `api/index.py:1451` · **Frontend:** `src/App.jsx:656`

Prompt `build_summary_system_instruction()` (`api/index.py:937-943`). Overwrites stored
...
**Handler:** `api/index.py:1493` · **Frontend:** `src/App.jsx` (`regenerateScript`)
...
- **Requested style never generated** → calls Gemini with
  `build_podcast_system_instruction()` (`api/index.py:947-959`) — same style guidance and
...
**File/line:** `api/index.py:1011-1048` (shared by §1.3 and §1.7 — both the initial analysis
...
the segment-audio handler (`api/index.py:1991`).

**Endpoint:** `POST /api/podcast/segment-audio` — handler at `api/index.py:1957`
...
A cache check runs first (`api/index.py:1967`): if `document_id` + `segment_index` are given
...
ad-hoc preview), the handler (`api/index.py:1998-2023`) fetches that document's full
...
handler falls back to a single-segment call (`api/index.py:2025-2034`). The ElevenLabs path
...
segment by `_split_pcm_by_chars()` (`api/index.py:1696-1717`) — boundaries proportional to
...
cached audio is **chunked**: `save_segment_audio()` (`api/index.py:539`) splits anything
over `MAX_CACHED_AUDIO_BYTES` = 740,000 raw bytes (`api/index.py:88`) across sibling chunk
documents (`_audio_chunk_ref()`, `api/index.py:495`) and `get_cached_segment_audio()`
(`api/index.py:513`) reassembles them; a missing chunk reads as a cache miss.
...
- Auth: `x-goog-api-key` header — `GOOGLE_TTS_API_KEY`, falling back to `GEMINI_API_KEY` (`get_google_tts_api_key()`, `api/index.py:337`). **The Cloud Text-to-Speech API must be enabled on the key's Google Cloud project** (the 403 error message links to the enable page). WaveNet pricing: $4/1M chars, 1M-chars/month free tier.
...
defensive `GOOGLE_TTS_MAX_INPUT_BYTES` = 4,800 budget (`api/index.py:69`) — multi-byte
...
### 2.2 Gemini TTS (opt-in, `TTS_PROVIDER=gemini`) — `gemini_tts()`, `api/index.py:1646-1693`
...
- Batch generation: `_tts_batch_bounds()` (`api/index.py:1849-1868`) groups consecutive
  segments into ≤`GEMINI_TTS_BATCH_CHARS` batches; `gemini_tts_batch()`
  (`api/index.py:1871-1955`) builds one "SpeakerName: line" block per batch and uses
...
browsers can't play directly, so `pcm_to_wav()` (`api/index.py:1586-1600`) wraps it in a WAV
header (sample rate parsed from the mimeType). **Output:** `audio/wav` bytes.

### 2.3 ElevenLabs (opt-in, `TTS_PROVIDER=elevenlabs`) — `elevenlabs_tts()`, `api/index.py:1603-1643`
...
- Voices resolved from the account via `resolve_voice_ids()` (`api/index.py:350-379`; avoids the free-tier 402 on library voices), cached in-process (`_cached_voice_ids`, `api/index.py:347`); `ELEVENLABS_VOICE_HOST_A/B` override when both set
...
- **Backend:** `require_user()` (`api/index.py:434-456`) — FastAPI dependency (`Depends(require_user)`). Reads `Authorization: Bearer <idToken>`, verifies via `firebase_auth.verify_id_token()`, enforces the `ALLOWED_EMAILS` allowlist (403), returns `AuthedUser {uid, email}` (`api/index.py:300-302`). Admin init: `get_firebase_app()` (`api/index.py:388-409`).
...
(`api/index.py:415-431`). Layout under `users/{uid}/documents/{doc_id}`:

| Path | Written by | File/line | Contents |
|---|---|---|---|
| `documents/{doc_id}` | `analyze_pdf()` | `api/index.py:1230` | `title`, `file_name`, `summary`, `quiz`, `podcast` (active script), `podcast_style` (active style), `podcast_versions` (`{style: script + audio_ns}` — every generated style version, ≤4,500 chars each, reused on style switch per §1.7), `document_context` (≤`MAX_STORED_CONTEXT_BYTES` = 900 KB, `api/index.py:102`), `created_at`. `summary`/`quiz` are overwritten in place by §1.5–1.6 |
| `documents/{doc_id}/audio/{style}.{segment_index}` | `save_segment_audio()` | `api/index.py:539` | `{ "data": "<base64 audio>", "mime": "audio/wav" \| "audio/mpeg", "chunks": N }` — one doc per segment plus a `{style}.full` sentinel doc holding the single continuous MP3 episode track (§2.1), **namespaced per podcast style** (`_audio_doc_id()`, `api/index.py:470`) so every generated style keeps its own audio; pre-versioning audio lives under plain integer IDs (the `"legacy"` namespace, still served). Audio over `MAX_CACHED_AUDIO_BYTES` = 740 KB raw (`api/index.py:88`) is split across sibling chunk docs `….c{n}` (`{ "data": ... }` only) and reassembled on read by `get_cached_segment_audio()` (`api/index.py:513`); a missing chunk reads as a cache miss. Fresh regeneration of a style clears only that style's namespace (§1.7); document deletion drops the whole subcollection |
| `documents/{doc_id}/chat/log` | `save_chat_log()` | `api/index.py:1356` | `{ "messages": [...] }` — last `MAX_STORED_CHAT_MESSAGES` = 60, each text ≤`MAX_STORED_CHAT_TEXT_BYTES` = 10 KB (`api/index.py:98-99`) |
| `documents/{doc_id}/quiz_attempts/{auto_id}` | `save_quiz_attempt()` | `api/index.py:586-619` | `{ questions, answers, score, total, created_at }` — score computed server-side; capped at `MAX_QUIZ_ATTEMPTS` = 20 (`api/index.py:105`) |
| `usage/{yyyy-mm-dd}` | `increment_usage()` | `api/index.py:685-690` | `{ count, date }` — shared daily counter across analyze/chat/quiz-/summary-/podcast-regenerate/audio-generate |

Reads: `list_documents()` (`api/index.py:1130`), `get_document()` (`api/index.py:1160`),
`get_cached_segment_audio()` (`api/index.py:513`, returns bytes + MIME; legacy docs without
`mime` default to `audio/mpeg`), `list_cached_segment_indices()` (`api/index.py:647-668`),
`get_chat_log()` (`api/index.py:1342`), `list_quiz_attempts()` (`api/index.py:622-646`).
Deletes cascade to `audio`/`chat`/`quiz_attempts` via `_delete_document_and_subcollections()`
(`api/index.py:1181-1191`). Two small shared helpers back the regenerate endpoints:
`_get_document_or_404()` (`api/index.py:1432-1438`) and `_require_document_context()`
(`api/index.py:1441-1448`).
...
|---|---|---|---|---|
| GET | `/api/health` | none | `api/index.py:1090` | — (reports `tts_provider`) |
| GET | `/api/profile` | required | `api/index.py:1107` | Firestore |
| GET | `/api/documents` | required | `api/index.py:1130` | Firestore |
| GET | `/api/documents/{doc_id}` | required | `api/index.py:1160` | Firestore |
| DELETE | `/api/documents/{doc_id}` | required | `api/index.py:1194` | Firestore |
| DELETE | `/api/documents` | required | `api/index.py:1203` | Firestore |
| POST | `/api/pdf/prepare` | none | `api/index.py:1212` | — |
| POST | `/api/pdf/analyze` | required | `api/index.py:1230` | Gemini, Firestore |
| POST | `/api/chat` | required | `api/index.py:1308` | Gemini |
| GET | `/api/documents/{doc_id}/chat` | required | `api/index.py:1342` | Firestore |
| PUT | `/api/documents/{doc_id}/chat` | required | `api/index.py:1356` | Firestore |
| GET | `/api/documents/{doc_id}/quiz/attempts` | required | `api/index.py:1373` | Firestore |
| POST | `/api/documents/{doc_id}/quiz/attempts` | required | `api/index.py:1378` | Firestore |
| POST | `/api/documents/{doc_id}/quiz/regenerate` | required | `api/index.py:1389` | Gemini, Firestore |
| POST | `/api/documents/{doc_id}/summary/regenerate` | required | `api/index.py:1451` | Gemini, Firestore |
| POST | `/api/documents/{doc_id}/podcast/regenerate` | required | `api/index.py:1493` | Gemini, Firestore |
| GET | `/api/podcast/audio-status/{doc_id}` | required | `api/index.py:1561` | Firestore |
| POST | `/api/podcast/episode-audio` | required | `api/index.py:2037` | Google Cloud TTS (whole episode as ONE MP3 track, on cache miss), Firestore |
| POST | `/api/podcast/segment-audio` | required | `api/index.py:1957` | Google Cloud TTS (whole episode, 1 call) **or** Gemini TTS (≤2,250-char batches) **or** ElevenLabs (1 call/segment) — on cache miss only — plus Firestore |
