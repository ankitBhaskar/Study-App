...
- Shared text transport: `call_gemini()` — `api/index.py:935-983`

### 1.1 Shared transport — `call_gemini()`

**File/line:** `api/index.py:935`

**Request built** (`api/index.py:946-959`):
...
Response parsed at `api/index.py:974-983`. Raises `HTTPException(502)` on non-200,
unexpected shape, or empty completion.

### 1.2 Shared style/length guidance

`PODCAST_STYLE_GUIDANCE` (`api/index.py:1006`) and `SUMMARY_LENGTH_GUIDANCE`
(`api/index.py:1024`) back both the combined analysis prompt and the standalone regenerate
...
**Handler:** `api/index.py:1427` · **Frontend:** `src/App.jsx:329`

Multipart form: `file: UploadFile` (PDF ≤4 MB, `read_pdf_upload()` at `api/index.py:1239`)
plus optional `podcast_style` / `summary_length` / `summary_focus` Form fields and the
`Authorization` header.

**Prompt** built by `build_study_system_instruction()` (`api/index.py:1044-1074`) — a single
...
Called at `api/index.py:1462`. Normalised by `normalise_study_content()`
(`api/index.py:1221`) via `parse_summary_points()` (`api/index.py:1236`),
`parse_quiz_questions()` (`api/index.py:1149`), `parse_podcast_script()` (`api/index.py:1181`,
see §1.8 for the character-cap enforcement).

**Output — `StudyAnalysisResponse`, `api/index.py:202-220`:**
...
**Handler:** `api/index.py:1505` · **Frontend:** `src/App.jsx:1318` (`TutorPanel.send()`)
...
**Input — `ChatRequest`, `api/index.py:228-232`** · **Output — `ChatResponse`, `api/index.py:235-236`** (`{ "answer": "string" }`).

### 1.5 Quiz regeneration (`POST /api/documents/{doc_id}/quiz/regenerate`)

**Handler:** `api/index.py:1586` · **Frontend:** `src/App.jsx:801`

Prompt `QUIZ_SYSTEM_INSTRUCTION` (`api/index.py:1102`). Avoid-list = current quiz + last 5
attempts' questions, capped at `MAX_AVOID_QUESTIONS` = 30 (`api/index.py:107`). Overwrites
stored `quiz`. Output `QuizRegenerateResponse` (`api/index.py:270-271`).

### 1.6 Summary regeneration (`POST /api/documents/{doc_id}/summary/regenerate`)

**Handler:** `api/index.py:1696` · **Frontend:** `src/App.jsx:656`

Prompt `build_summary_system_instruction()` (`api/index.py:1077-1084`). Overwrites stored
`summary`. Input `SummaryRegenerateRequest` (`api/index.py:293-295`, `{length, focus}`),
output `SummaryRegenerateResponse` (`api/index.py:298-299`).

### 1.7 Podcast style switch / regeneration (`POST /api/documents/{doc_id}/podcast/regenerate`)

**Handler:** `api/index.py:1738` · **Frontend:** `src/App.jsx` (`regenerateScript`)
...
- **Requested style never generated** → calls Gemini with
  `build_podcast_system_instruction()` (`api/index.py:1087-1099`) — same style guidance and
...
Input `PodcastRegenerateRequest` (`api/index.py:302-303`, `{style}`), output
`PodcastRegenerateResponse` (`api/index.py:306-312`,
...
**File/line:** `api/index.py:1181-1218` (shared by §1.3 and §1.7 — both the initial analysis
...
**Handlers:** `api/index.py:1629` (list, storage only) and `api/index.py:1629` (generate) ·
**Frontend:** `src/App.jsx` (`FlashcardsPanel`, the "Cards" tab).

Prompt `FLASHCARD_SYSTEM_INSTRUCTION` (`api/index.py:1119`) asks for EXACTLY
`FLASHCARDS_PER_SET` = 6 cards (`api/index.py:110`) as `{"cards": [{"front", "back"}]}`,
grounded in the document. Parsed/enforced by `parse_flashcards()` (`api/index.py:1133`):
...
under `flashcard_sets/{auto_id}` (`save_flashcard_set()`, `api/index.py:743`), newest first
...
the segment-audio handler (`api/index.py:2236`).

**Endpoint:** `POST /api/podcast/segment-audio` — handler at `api/index.py:2202`
...
A cache check runs first (`api/index.py:2221`): if `document_id` + `segment_index` are given
...
ad-hoc preview), the handler (`api/index.py:2243-2268`) fetches that document's full
...
handler falls back to a single-segment call (`api/index.py:2270-2279`). The ElevenLabs path
...
segment by `_split_pcm_by_chars()` (`api/index.py:1941-1962`) — boundaries proportional to
...
cached audio is **chunked**: `save_segment_audio()` (`api/index.py:625`) splits anything
over `MAX_CACHED_AUDIO_BYTES` = 740,000 raw bytes (`api/index.py:89`) across sibling chunk
documents (`_audio_chunk_ref()`, `api/index.py:581`) and `get_cached_segment_audio()`
(`api/index.py:599`) reassembles them; a missing chunk reads as a cache miss.
...
- Auth: `x-goog-api-key` header — `GOOGLE_TTS_API_KEY`, falling back to `GEMINI_API_KEY` (`get_google_tts_api_key()`, `api/index.py:386`). **The Cloud Text-to-Speech API must be enabled on the key's Google Cloud project** (the 403 error message links to the enable page). Pricing: Neural2 $16/1M chars, WaveNet $4/1M, ~1M free chars/month.
...
### 2.2 Gemini TTS (opt-in, `TTS_PROVIDER=gemini`) — `gemini_tts()`, `api/index.py:1891-1938`
...
- Batch generation: `_tts_batch_bounds()` (`api/index.py:2180-2199`) groups consecutive
  segments into ≤`GEMINI_TTS_BATCH_CHARS` batches; `gemini_tts_batch()`
  (`api/index.py:2116-2199`) builds one "SpeakerName: line" block per batch and uses
...
browsers can't play directly, so `pcm_to_wav()` (`api/index.py:1831-1845`) wraps it in a WAV
header (sample rate parsed from the mimeType). **Output:** `audio/wav` bytes.

### 2.3 ElevenLabs (opt-in, `TTS_PROVIDER=elevenlabs`) — `elevenlabs_tts()`, `api/index.py:1848-1888`
...
- Voices resolved from the account via `resolve_voice_ids()` (`api/index.py:436-465`; avoids the free-tier 402 on library voices), cached in-process (`_cached_voice_ids`, `api/index.py:436`); `ELEVENLABS_VOICE_HOST_A/B` override when both set
...
**Input to the endpoint — `SegmentAudioRequest`, `api/index.py:315-322`:**
...
- **Backend:** `require_user()` (`api/index.py:520-542`) — FastAPI dependency (`Depends(require_user)`). Reads `Authorization: Bearer <idToken>`, verifies via `firebase_auth.verify_id_token()`, enforces the `ALLOWED_EMAILS` allowlist (403), returns `AuthedUser {uid, email}` (`api/index.py:336-338`). Admin init: `get_firebase_app()` (`api/index.py:138-142`).
...
(`api/index.py:501-517`). Layout under `users/{uid}/documents/{doc_id}`:

| Path | Written by | File/line | Contents |
|---|---|---|---|
| `documents/{doc_id}` | `analyze_pdf()` | `api/index.py:1427` | `title`, `file_name`, `summary`, `quiz`, `podcast` (active script), `podcast_style` (active style), `podcast_versions` (`{style: script + audio_ns}` — every generated style version, ≤4,500 chars each, reused on style switch per §1.7), `document_context` (≤`MAX_STORED_CONTEXT_BYTES` = 900 KB, `api/index.py:103`), `created_at`. `summary`/`quiz` are overwritten in place by §1.5–1.6 |
| `documents/{doc_id}/audio/{style}.{segment_index}` | `save_segment_audio()` | `api/index.py:625` | `{ "data": "<base64 audio>", "mime": "audio/wav" \| "audio/mpeg", "chunks": N }` — one doc per segment plus a `{style}.full` sentinel doc holding the single continuous MP3 episode track (§2.1), **namespaced per podcast style** (`_audio_doc_id()`, `api/index.py:556`) so every generated style keeps its own audio; pre-versioning audio lives under plain integer IDs (the `"legacy"` namespace, still served). Audio over `MAX_CACHED_AUDIO_BYTES` = 740 KB raw (`api/index.py:89`) is split across sibling chunk docs `….c{n}` (`{ "data": ... }` only) and reassembled on read by `get_cached_segment_audio()` (`api/index.py:599`); a missing chunk reads as a cache miss. Fresh regeneration of a style clears only that style's namespace (§1.7); document deletion drops the whole subcollection |
| `documents/{doc_id}/chat/log` | `save_chat_log()` | `api/index.py:1553` | `{ "messages": [...] }` — last `MAX_STORED_CHAT_MESSAGES` = 60, each text ≤`MAX_STORED_CHAT_TEXT_BYTES` = 10 KB (`api/index.py:100-100`) |
| `documents/{doc_id}/quiz_attempts/{auto_id}` | `save_quiz_attempt()` | `api/index.py:672-705` | `{ questions, answers, score, total, created_at }` — score computed server-side; capped at `MAX_QUIZ_ATTEMPTS` = 20 (`api/index.py:106`) |
| `documents/{doc_id}/flashcard_sets/{auto_id}` | `save_flashcard_set()` | `api/index.py:743` | `{ cards: [{front, back}], created_at }` — one document per generated set of 6 cards; every set is kept (newest first), capped at `MAX_FLASHCARD_SETS` = 10 (`api/index.py:111`). Deleted with the document |
| `feedback/{auto_id}` | `submit_feedback()` | `api/index.py:1302-1322` | `{ uid, email, rating (1-5), comment (≤2,000 chars), context (≤100 chars), created_at }` — one document per "Give feedback" submission (§5). **Top-level collection, not under `users/{uid}`** — it's an app-wide feedback log for the maintainer, so it's unaffected by a user deleting their own documents/history |
| `usage/{yyyy-mm-dd}` | `increment_usage()` | `api/index.py:825-830` | `{ count, date }` — shared daily counter across analyze/chat/quiz-/summary-/podcast-regenerate/audio-generate |

Reads: `list_documents()` (`api/index.py:1325`), `get_document()` (`api/index.py:1355`),
`get_cached_segment_audio()` (`api/index.py:599`, returns bytes + MIME; legacy docs without
`mime` default to `audio/mpeg`), `list_cached_segment_indices()` (`api/index.py:787-808`),
`get_chat_log()` (`api/index.py:1539`), `list_quiz_attempts()` (`api/index.py:708-730`).
Deletes cascade to `audio`/`chat`/`quiz_attempts`/`flashcard_sets` via
`_delete_document_and_subcollections()` (`api/index.py:1376-1388`). Two small shared helpers
back the regenerate endpoints: `_get_document_or_404()` (`api/index.py:1677-1683`) and
`_require_document_context()` (`api/index.py:1677-1693`).

---

## 5. Resend (`api.resend.com`) — feedback email notifications

Optional, best-effort. `POST /api/feedback` (`api/index.py:1301-1322`) always saves the
...
- Auth: `Authorization: Bearer <RESEND_API_KEY>` (`api/index.py:134`) — unset means the
  feature is off entirely, `send_feedback_email()` returns `False` immediately without
  making a request (`api/index.py:396-403`).
- Destination: `FEEDBACK_EMAIL_TO` (`api/index.py:135`) — also required, or email is skipped.
- Sender: `FEEDBACK_EMAIL_FROM` (`api/index.py:136`), defaults to Resend's shared sandbox
...
(`api/index.py:428-430`) — `submit_feedback()` never raises because of the email step,
...
| PUT | `/api/documents/{doc_id}/chat` | required | `api/index.py:1553` | Firestore |
| GET | `/api/documents/{doc_id}/quiz/attempts` | required | `api/index.py:1570` | Firestore |
| POST | `/api/documents/{doc_id}/quiz/attempts` | required | `api/index.py:1575` | Firestore |
| POST | `/api/documents/{doc_id}/quiz/regenerate` | required | `api/index.py:1586` | Gemini, Firestore |
| GET | `/api/documents/{doc_id}/flashcards` | required | `api/index.py:1629` | Firestore |
| POST | `/api/documents/{doc_id}/flashcards/regenerate` | required | `api/index.py:1635` | Gemini, Firestore |
| POST | `/api/documents/{doc_id}/summary/regenerate` | required | `api/index.py:1696` | Gemini, Firestore |
| POST | `/api/documents/{doc_id}/podcast/regenerate` | required | `api/index.py:1738` | Gemini, Firestore |
| GET | `/api/podcast/audio-status/{doc_id}` | required | `api/index.py:1806` | Firestore |
| POST | `/api/podcast/episode-audio` | required | `api/index.py:2282` | Google Cloud TTS (whole episode as ONE MP3 track, on cache miss), Firestore |
| POST | `/api/podcast/segment-audio` | required | `api/index.py:2202` | Google Cloud TTS (whole episode, 1 call) **or** Gemini TTS (≤2,250-char batches) **or** ElevenLabs (1 call/segment) — on cache miss only — plus Firestore |
