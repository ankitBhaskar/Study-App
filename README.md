# Syrora — Knowledge that adapts to every mind.
...
- `GEMINI_API_KEY` — from <https://aistudio.google.com/apikey>, required for study content generation; also used for podcast audio (Google Cloud TTS by default — enable it at <https://console.cloud.google.com/apis/library/texttospeech.googleapis.com> for the key's project, or set a separate `GOOGLE_TTS_API_KEY`)
...
- `GEMINI_TTS_VOICE_A` / `GEMINI_TTS_VOICE_B` — optional prebuilt Gemini voices for the two hosts (default: `Kore` / `Puck`)
- `ELEVENLABS_MODEL` — optional, defaults to `eleven_multilingual_v2` (only used with `TTS_PROVIDER=elevenlabs`)
...
- `GET /api/documents/{id}/flashcards` / `POST /api/documents/{id}/flashcards/regenerate` — list every saved flashcard set (newest first, capped at 10) or generate a fresh set of 6 cards grounded in the document, avoiding recent cards' fronts. **Old sets are never deleted by generating a new one** — the Cards tab switches between them. Listing is storage-only; generation requires `GEMINI_API_KEY` and counts against the daily usage limit.
...
- `POST /api/podcast/segment-audio` — turn one transcript segment into speech (`{text, speaker, document_id, segment_index}`; the last two are optional but enable caching). Uses the `TTS_PROVIDER` backend: Google Cloud TTS (default) and Gemini TTS return WAV, ElevenLabs returns MP3. With the default Google backend the WHOLE episode is synthesized in one fast Cloud TTS call on the first cache miss and every segment is cached at once. Counts against the daily usage limit only on a cache miss. The frontend calls this once per segment and plays them back-to-back, keeping each response well under Vercel's size limits. When `document_id`/`segment_index` are given, generated audio is stored (base64 + its MIME type, one Firestore document per segment) and reused on future requests — free and instant, no new TTS call or usage-limit hit. (WAV segments above ~740 KB are chunked across multiple Firestore documents to stay under the 1 MiB per-document limit.)
