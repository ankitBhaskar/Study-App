...
- `GEMINI_API_KEY` — from <https://aistudio.google.com/apikey>, required for study content generation **and** the default Gemini TTS podcast audio
- `ELEVENLABS_API_KEY` — from <https://elevenlabs.io> (Profile → API Keys). Only needed if you set `TTS_PROVIDER=elevenlabs`; podcast audio defaults to Gemini TTS, which needs no extra key
...
- `TTS_PROVIDER` — optional, `gemini` (default, cheaper) or `elevenlabs`; picks the podcast text-to-speech backend
...
- `POST /api/podcast/segment-audio` — turn one transcript segment into speech (`{text, speaker, document_id, segment_index}`; the last two are optional but enable caching). Uses the `TTS_PROVIDER` backend: Gemini TTS returns WAV, ElevenLabs returns MP3. Counts against the daily usage limit only on a cache miss. The frontend calls this once per segment and plays them back-to-back, keeping each response well under Vercel's size limits. When `document_id`/`segment_index` are given, generated audio is stored (base64 + its MIME type, one Firestore document per segment) and reused on future requests — free and instant, no new TTS call or usage-limit hit. (Gemini WAV segments above ~740 KB are served but not cached, to stay under Firestore's 1 MiB document limit.)
